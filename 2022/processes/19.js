...
module.exports = 
{
    run(data)
    {
        ...
        const testBlueprint = (blueprint, time) => 
        {
            ...
            {
                if (time < 1 || geodes + (time * (time + 1)) / 2 < maxGeode) return;
                ...
                if (obsidianRobots > 0) 
                {
                    ...
                    let timeSkip = 1 + (canBuildGeodeNow ? 0 : Math.max( Math.ceil((blueprint.geodeCost[0] - ore) / oreRobots), Math.ceil((blueprint.geodeCost[1] - obsidian) / obsidianRobots) ));
                ...
                if (clayRobots > 0) 
                {
                    let canBuildObsidianNow = blueprint.obsidianCost[0] <= ore && blueprint.obsidianCost[1] <= clay;
                    let timeSkip = 1 + (canBuildObsidianNow ? 0 : Math.max(  Math.ceil((blueprint.obsidianCost[0] - ore) / oreRobots), Math.ceil((blueprint.obsidianCost[1] - clay) / clayRobots)));

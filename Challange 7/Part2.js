...
input.split("\n").filter((line) => line).map((rule_enc) =>  {
	var [out_enc, in_enc] = rule_enc.split(" bags contain ");
	
	var innerbags = in_enc.split(", ").map((in_enc) =>  {
		return decodebag(in_enc)
	})
	.filter((innerbag) => innerbag)
	
	var colour = out_enc.trim();
...
{
	if(enc.indexOf('no other bags.') !== -1) return null;

	[, card, colour] = /([0-9])\s(([a-z]+\s){2})bag(s?)/.exec(enc);
	return {
		card: 1 * (card.trim()),
		colour: colour.trim(),
	}
...
function findreqbags(colour)
{
	var rootbag = bagMap[colour];
	
	return rootbag.innerbags.reduce((count, innerbag) => {
		var innercount = findreqbags(innerbag.colour);
		return count + innerbag.card + innerbag.card * innercount;
	}, 0)
}

var reqbagscount = findreqbags('shiny gold')

console.log(reqbagscount);
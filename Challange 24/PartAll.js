...
function part2(data)
{
	return Array.from({ length: 100 }).reduce((acc) => flip(acc), getBlackTiles(input)).size
}

var move = ([x, y], dir) => [x + coord_map[dir][0], y + coord_map[dir][1]];

function getBlackTiles(data)
{
	return data.split("\n").reduce((acc, line) => {
		var position = line.match(/(e|se|sw|w|nw|ne)/g).reduce(move, [0, 0]);
...
{
	var position = tile.split(";").map(Number);
	return Object.keys(coord_map).map((x) => move(position, x).join(";"));
};

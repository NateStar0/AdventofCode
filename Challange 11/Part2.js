...
function getType(x, y, a, posarr)
{
	var line = a[y];
	if((x < 0) || (x >= width) || (y < 0) || (y >= height)) return " ";
	var type = line.charAt(x);
	
	while(type == ".")
	{
		//Continue going
		//posarr
		//console.log(x, y, posarr, type)
		if(x != posarr[0])
		{
			if(x < posarr[0]) x -= 1;
			if(x > posarr[0]) x += 1;
		...
		//console.log(x, y, posarr, type, work)
	}
	...
	for(var k = 0; k < height; k++)
	{
		for(var j = 0; j < width; j++)
		{
			...
			if(chr != ".")
			{
				...
				neigh += getType(j - 1, k - 1, arr, [j, k])	//Diag top left
				neigh += getType(j + 1, k - 1, arr, [j, k])	//Diag top right
				neigh += getType(j - 1, k + 1, arr, [j, k])	//Diag bottom left
				neigh += getType(j + 1, k + 1, arr, [j, k])	//Diag bottom right
					
				neigh += getType(j, k - 1, arr, [j, k])	//Top
				neigh += getType(j, k + 1, arr, [j, k])	//Bottom
				neigh += getType(j - 1, k, arr, [j, k])	//Left
				neigh += getType(j + 1, k, arr, [j, k])	//right

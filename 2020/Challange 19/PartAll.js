...
function part1(data)
{
	var chop = data.split("\n\n"); //Split into rules and checks
	var rules = chop[0].split("\n"); //Split into individual rules
	
	var rule_col = {}; // A better collection for the rules
	
	rules = rules.sort((b1, b2) => b1.split(": ")[0] - b2.split(": ")[0]) //Sort from 0 - max
	rules.forEach((rule) => //Convert the array into a map
	{
		const {groups} = /^(?<key>\d+): (?<value>.*)$/.exec(rule);
		rule_col[groups.key] = groups.value;
	})
	
	var check = chop[1].split("\n") //Get the values to check
	var ruleRegexConv = {};
	
	function process(val, rules)
	{
		if(val in ruleRegexConv)
		{
			return ruleRegexConv[val]
		}
		
		var res = '';
		
		if(/^".*"$/.test(val)) //Does this contain a charcter
		{
			res = val.replace(/"/g, '');
		} 
		else if(/\|/.test(val)) //Does this have an or bar?
		{
			var opts = val.split(' | ');
			res = `(${process(opts[0], rules)}|${process(opts[1], rules)})`;
		}
		else  //This is neither an end nor a split.
		{
			var ks = val.split(' ');
			res = ks.map(k => process(rules[k], rules)).join('');
		}
		
		ruleRegexConv[val] = res;
		return res;
	}
	
	function tally(rule, check)
	{
		var t = 0;
		for(var i = 0; i < check.length; i++)
		{
			if(rule.test(check[i]))
			{
				t += 1;
			}
		}
	...
	process(rule_col[0], rule_col);
	
	var central_rule = new RegExp('^' + ruleRegexConv[rule_col[0]] + '$');
	return tally(central_rule, check); //Return the valid number for logging
}
...
function part2(data)
{
	...
	function tally(rule, check)
	{
		...
		for(var i = 0; i < check.length; i++)
		{
			...
			{
				var {groups} = m;
				var a = groups.collection42.match(new RegExp(ruleRegexConv[rules[42]], 'g')).length;
				var b = groups.collection31.match(new RegExp(ruleRegexConv[rules[31]], 'g')).length;
	...
	//console.log(rule_col)
	
	rule_col['0'] = '8 11';
	rule_col['8'] = '42 | 42 8'; // 42 at least one time
	rule_col['11'] = '42 31 | 42 11 31'; // 42{n}31{n}
	
	//console.log(rule_col, rule_col[42], rule_col[31])

	process(rule_col[42], rule_col);
	process(rule_col[31], rule_col);
	
	var central_rule = new RegExp("^(?<collection42>(" + ruleRegexConv[rule_col[42]] + ")+)(?<collection31>(" + ruleRegexConv[rule_col[31]] + ")+)$");
	var count = tally(central_rule, check);
	return count; //Return the valid number for logging
}

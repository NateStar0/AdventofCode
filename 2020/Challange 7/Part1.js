...
function decodebag(enc)
{
	...
	[noth, card, colour] = /([0-9])\s(([a-z]+\s){2})bag(s?)/.exec(enc);
	return {
		card: 1 * (card.trim()),
		colour: colour.trim(),
	}
...
function findoptions(out_bag)
{
	var req = [];
	var children = findChildren(out_bag);
	req = req.concat(children);
	
	children.forEach((childbag) => {
		var neste = findoptions(childbag.colour);
		if(neste && neste.length)
		{
			req = req.concat(neste);
			req = [...new Set(req)];
		}
	})
...
{
	var cancont = bags.filter((bag) => {
		return bag.innerbags.some((innerbag) => {
			if(innerbag != null )
			{
				return innerbag.colour == colour && innerbag.card;
			}
		})
	})
...
var leng = findoptions('shiny gold').length
console.log(leng);
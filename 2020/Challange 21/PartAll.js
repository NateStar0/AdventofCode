...
function part1_var(data)
{
	...
	{
		var ingncont_split = list.split(" (contains ");
		var contains = ingncont_split[1].split(")")[0].split(", ");
		
		var words = ingncont_split[0].split(" ");
		words.forEach((word) => 
		{
			word_bank.push([word, contains])
		})
		//console.log(words, contains)
	})
	
	//Remove extra entries, if an allergen does not appear every time, cull.
	word_bank.sort(); //Groups them together

	var cleaned_arr = new Array(word_bank.length);
	for(var k = 0; k < cleaned_arr.length; k++) cleaned_arr[k] = new Array(0)
	var j = 0;
	var current = word_bank[0][0];
	for(var i = 0; i < word_bank.length; i++)
	{
		if(word_bank[i][0] == current)
		{
			cleaned_arr[j].push(word_bank[i]);
		}
		else
		{
			j++
			current = word_bank[i][0]
			cleaned_arr[j].push(word_bank[i]);
		}
	}
	
	var correct = [];
	cleaned_arr.forEach((word_clump) =>
	{
		var contain_arr = [];
		var contain_n = word_clump.length;
		for(var i = 0; i < word_clump.length; i++)
		{
			contain_arr.push(word_clump[i][1])
		}
		
		var check = [0, 0, 0, 0, 0, 0, 0, 0] 
		var map = ["nuts", "dairy", "soy", "wheat", "fish", "shellfish", "peanuts", "sesame"];
		for(var j = 0; j < contain_arr.length; j++)
		{
			for(var k = 0; k < contain_arr[j].length; k++)
			{
				if(contain_arr[j][k] == "nuts") check[0] += 1;
				if(contain_arr[j][k] == "dairy") check[1] += 1;
				if(contain_arr[j][k] == "soy") check[2] += 1;
				if(contain_arr[j][k] == "wheat") check[3] += 1;
				
				if(contain_arr[j][k] == "fish") check[4] += 1;
				if(contain_arr[j][k] == "shellfish") check[5] += 1;
				if(contain_arr[j][k] == "peanuts") check[6] += 1;
				if(contain_arr[j][k] == "sesame") check[7] += 1;
			}
		...
		{
			if(check[l] == word_clump.length && word_clump.length > 0)
			{
				b.push(map[l])
			}
		}
		
		if(word_clump[0] != undefined)correct.push([word_clump[0][0], b])
	})
	...
	for(var y = 0; y < correct.length; y++)
	{
		...
		if(correct[y][1].length < 1)
		{
			sum += count(data, correct[y][0])
		}
	}
...
function part_all(data)
{
	...
	{
		var m = /^([(\w+) ]+) \(contains ([\w+), ]+)\)$/.exec(products[i]);
		var fi = m[1].split(" ");
		var fa = m[2] ? m[2].split(', ') : [];
		...
		for(var l = 0; l < fi.length; l++)
		{
			if(p2.has(fi[l]))
			{
				var aset = new Set([...p2.get(fi[l]), ...fa])
				p2.set(fi[l], aset);
			}
			else
			{
				p2.set(fi[l], new Set(fa))
			}
		}	
	}
	
	var ayyokay = new Set(ing)
	for(var [__, il] of alle)
	{
		il.forEach((i) => ayyokay.delete(i));
		//console.log(ayyokay)
	}

	var res1 = 0;
	for (var ingre of ayyokay) 
	{
		for(var g = 0; g < foods.length; g++)
		{
			if(foods[g].includes(ingre)) res1 += 1;
		}
	}

	for (var [al, is] of alle) 
	{
		alle.set(al, new Set([...is].filter((i) => !ayyokay.has(i))));
	}
	
	var clean = [];
	while (alle.size > 0) {
	  for (const [al, is] of alle) {
		if (is.size === 1) {
		  var ingre = [...is][0];
		  clean.push({
			al,
	...
	clean.sort((a, b) => a.al.localeCompare(b.al));
	var res2 = clean.map((a) => a.ingre).join(',');
...
console.log("Part 1: ", part_all(input)[0]);
console.log("Part 2: ", part_all(input)[1]);

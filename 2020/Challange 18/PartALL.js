...
function part1(data)
{
	...
	function sum(ex) 
	{
		...
		while (ex.match(/\(/)){
			ex = ex.replace(/\([^()]+\)/, match => 
			sum(match.slice(1, match.length - 1))
			)
		}

		while (isNaN(Number(ex))) 
		{
			ex = ex.replace(/(\d+) ([+*]) (\d+)/, (match, a, op, b) => 
...
console.log("Part 2: ", part2(input)); //Two calls allow to diagnose any freezes

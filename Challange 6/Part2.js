...
var counted = 0;
work.forEach(count_answers);

function count_answers(item)
{
	
	var exclusive = removeDuplicateCharacters(item);
	exclusive = exclusive.replace(/\n/g, "")
	
	for(var i = 0; i < exclusive.length + 1; i++)
	{
		if(count(item, exclusive.charAt(i)) != count(item, "\n") + 1)
		{
			item = item.split(exclusive.charAt(i)).join("");
		}
	}
	
	item = item.replace(/\n/g, "")
	item = removeDuplicateCharacters(item);
	console.log(item, item.length, count(item, "\n") + 1);
	counted += item.length;
}
